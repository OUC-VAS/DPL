...
class CLIPIQA(nn.Module):
    def __init__(self,
        ...
        self.clip_model = [load(backbone, 'cpu', download_root='./pretrained/clip')]  # avoid saving clip weights
        ...
        if pretrained and 'clipiqa+' in model_type:
            if model_type == 'clipiqa+' and backbone == 'RN50':
                self.prompt_learner.ctx.data = torch.load(load_file_from_url(default_model_urls['clipiqa+'], model_dir='./pretrained/clip/'))
